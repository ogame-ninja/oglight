...
class OGLight {
    constructor(params) {
        var cookieAccounts=document.querySelector('head meta[name="ogame-player-id"]').getAttribute('content').replace(/\D/g, '');
    ...
    init(cache) {
        ...
        this.account.lang=lang, this.db.serverData = this.db.serverData || {}, 
        ...
        this.account.lang != this.db.userLang && "fleetdispatch" != this.page && "intro" != this.page ? window.location.href = `?page=ingame&component=fleetdispatch` : ("intro" != this.page && (this.db.userLang = this.account.lang), 
        this.cache = cache || {}, -1 < window.location.href.indexOf("&relogin=1") && (this.cache = {}), 
        this.updateJQuerySettings(), "empire" != this.page ? (this.id && this.id[0] || (cache = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
...
class FetchManager extends Manager {
    ...
    fetchEmpire() {
        ...
        for (let i = this.fetched.empire1 = 0; i <= 1; i++) fetch(`?page=ajax&component=empire&ajax=1&planetType=${i}&asJson=1`, {
            headers: {
    ...
    fetchLFBonuses() {
        ...
        fetch(`?page=ajax&component=lfbonuses`, {
            headers: {
                "X-Requested-With": "XMLHttpRequest"
            }
    ...
    fetchProductionQueue() {
        ...
        fetch(`?page=ajax&component=productionqueue&ajax=1`, {
            headers: {
                "X-Requested-With": "XMLHttpRequest"
            }
        }).then(response => response.text()).then(result => {
...
class UIManager extends Manager {
    ...
    attachGlobalClickEvent(reloaded) {
        reloaded || (document.addEventListener("keyup", event => {
            ...
            if ("svg" != event.target.tagName && "path" != event.target.tagName) {
                ...
                loadContentNew(galaxy, system)) : (coords = `?page=ingame&component=galaxy&galaxy=${coords[0]}&system=` + coords[1], 
                ...
                (event.target.classList.contains("planetlink") || event.target.classList.contains("moonlink") || event.target.closest(".planetlink, .moonlink")) && document.body.classList.contains("ogl_destinationPicker")) {
                    ...
                    if (document.body.classList.contains("ogl_initHarvest")) {
                        ...
                        window.location.href = `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${type}&oglmode=1&ogldestinationid=${planet}&oglfirstsourceid=${firstSource}&ogldestinationtype=` + (3 == type ? "moon" : "planet");
                    } else {
    ...
    getPlayerTooltip(id) {
        ...
        var page = Math.ceil(player.score.globalRanking / 100), page = Util.addDom("div", {
            ...
            child: `
            <h1 class="${this.ogl._highscore.tagToNameList[player.status] || "status_abbr_active"}">${player.name} <a href="${window.location.protocol}//${window.location.host}${window.location.pathname}?page=highscore&site=${page}&searchRelId=${id}">#${player.score.globalRanking}</a></h1>
            <div class="ogl_grid">
            ...
            onclick: () => {
                document.querySelector("#chatBar") || (window.location.href = `?page=chat&playerId=` + player.uid);
            }
        });
        ...
        writeIcon.setAttribute("data-playerId", player.uid)), Util.addDom("a", {
            ...
            parent: page.querySelector(".ogl_actions"),
            href: `?page=ingame&component=buddies&action=7&id=${player.uid}&ajax=1`,
            onclick: () => {
                tippy.hideAll();
            }
        }), Util.addDom("div", {
    ...
    addSpyIcons(parent, coords, uniqueType, displayActivity) {
        ...
        if (coords = "string" == typeof coords ? coords.split(":") : coords, "planet" != uniqueType && uniqueType || (planetIcon = Util.addDom("div", {
            ...
            child: "language",
            parent: parent,
            onclick: e => window.location.href = `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=1`
        ...
        "moon" == uniqueType || !uniqueType && this.ogl.db.pdb[`${coords[0]}:${coords[1]}:` + coords[2]]?.mid) {
            var lastPlanetSpy = 0 < this.ogl.db.pdb[`${coords[0]}:${coords[1]}:` + coords[2]]?.mid ? Util.addDom("div", {
                ...
                child: "bedtime",
                parent: parent,
                onclick: e => window.location.href = `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=3`
            }) : Util.addDom("div", {
                parent: parent
...
class TopbarManager extends Manager {
    load(reloaded) {
        ...
        Util.addDom("button", {
            ...
            onclick: () => {
                var nextID;
                this.ogl.currentPlanet.obj.planetID || this.ogl.currentPlanet.obj.moonID ? window.location.href = `?page=ingame&component=fleetdispatch&oglmode=2` : (nextID = "planet" == this.ogl.planetType ? this.ogl.currentPlanet.dom.nextWithMoon.getAttribute("id").replace("planet-", "") : this.ogl.currentPlanet.dom.nextWithMoon.querySelector(".moonlink").getAttribute("href").match(/cp=(\d+)/)[1], 
    ...
    addPinnedItemToList(player, list) {
        ...
        list = Util.addDom("a", {
            class: "ogl_ranking",
            href: `?page=highscore&site=${list}&searchRelId=` + player.uid,
        ...
        let updateSide = () => {
            ...
            if (player) {
                ...
                var writeIcon = Util.addDom("div", {
                    ...
                    parent: actions,
                    onclick: () => {
                        document.querySelector("#chatBar") || (window.location.href = `?page=chat&playerId=` + id);
                    }
                ...
                writeIcon.setAttribute("data-playerId", id)), Util.addDom("a", {
                    ...
                    href: `?page=ingame&component=buddies&action=7&id=${id}&ajax=1`,
                    onclick: () => {
                        tippy.hideAll();
                    }
                }), Util.addDom("div", {
                ...
                update || (Util.addDom("div", {
                    ...
                    class: "ogl_ranking",
                    href: `?page=highscore&site=${writeIcon}&searchRelId=` + player.uid,
                    child: "#" + player.score?.globalRanking || "?"
                });
...
class GalaxyManager extends Manager {
    ...
    updateRow(player, row, isOwn, coords) {
        ...
        Util.addDom("a", {
            ...
            href: `?page=highscore&site=${page}&searchRelId=` + player.uid,
            child: "#" + player.score.globalRanking
    ...
    addToSpyTable(message, wrapper) {
        if (!this.spytable.querySelector(`[data-id="${message.id}"]`)) {
            ...
            for (let i = 1; i < 7; i++) {
                ...
                this.ogl.fretShips.forEach(shipID => {
                    ...
                    Util.addDom("a", {
                        ...
                        href: `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${message.targetTypeID}&mission=1&am${shipID}=${shipsCount}&oglmode=4&oglLazy=true`
                    });
                });
            }
            var wrapper = message.activity < 15 ? "*" : message.activity.toString().replace("60", "-"), time = this.ogl.db.options.useClientTime ? message.date.client : message.date.server, time = (Util.addDom("span", {
                ...
                href: `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${message.targetTypeID}&mission=1&am${this.ogl.db.options.defaultShip}=${delta}&oglmode=4`
            ...
            Util.addDom("div", {
                ...
                parent: bonusFleetSpeed,
                child: "target",
                href: `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${message.targetTypeID}&mission=1`
            }));
...
class TechManager extends Manager {
    ...
    openTodolist(data, coords, id) {
        ...
        }), `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&oglmode=3&targetid=${id}&type=` + coords[3]), sendButton = Util.addDom("button", {
