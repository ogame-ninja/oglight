...
}), void 0 === window?.GM_info && (window.GM_info = {}, window.GM_info.script = {}, window.GM_info.script.name = "OGLight IOS", window.GM_info.script.version = oglVersion, window.GM_info.script.downloadURL = "https://openuserjs.org/install/nullNaN/OGLight.user.js");
class OGLight {
  constructor(e) {
...
class TimeManager extends Manager {
  ...
  timeToHMS(e) {
    return new Date(e).toLocaleTimeString("de-DE", {
      hour12: !1
    })
  }
}
class FetchManager extends Manager {
  ...
  fetchProductionQueue() {
    this.ogl._topbar.syncBtn.classList.add("ogl_active"), this.fetched.prod = 0, fetch(`?page=ajax&component=productionqueue&ajax=1`, {
      headers: {
        "X-Requested-With": "XMLHttpRequest"
      }
    }).then((e => e.text())).then((e => {
      const t = (new DOMParser).parseFromString(e, "text/html"),
        n = t.querySelector(".research time");
      document.querySelectorAll(".planetlink, .moonlink").forEach((e => {
        const o = new URLSearchParams(e.getAttribute("href")).get("cp").split("#")[0],
          a = {
            baseBuilding: t.querySelector(`time.building${o}`),
            ship: t.querySelector(`time.ship${o}`),
            lfBuilding: t.querySelector(`time.lfbuilding${o}`),
            lfResearch: t.querySelector(`time.lfresearch${o}`),
            mechaShip: t.querySelector(`time.ship_2nd${o}`)
          };
        this.ogl.db.myPlanets[o].upgrades = this.ogl.db.myPlanets[o].upgrades || {};
        for (let [e] of Object.entries(a)) {
          if (a[e]) {
            const t = a[e].closest(".productionDetails")?.querySelector(".productionLevel")?.innerText?.replace(/\D/g, "") || "0",
              n = {};
            n.name = a[e].closest(".productionDetails").querySelector(".productionName").childNodes[0].textContent, n.lvl = parseInt(t), n.end = 1e3 * parseInt(a[e].getAttribute("data-end")), n.type = e, this.ogl.db.myPlanets[o].upgrades[e] = [n]
          }
          else this.ogl.db.myPlanets[o].upgrades[e] = [];
          n || delete this.ogl.db.myPlanets[o].upgrades.baseResearch
        }
...
class FleetManager extends Manager {
  ...
  overwrite() {
    this.overwrited = !0, fleetDispatcher.fleetHelper.getShipData = e => fleetDispatcher?.fleetHelper?.shipsData?.[e] || shipsData[e], fleetDispatcher.refreshDataAfterAjax = e => {
      ...
      a && (a.classList.add("tooltipClose"), a.classList.add("tooltipClick")), this.ogl._time.timeLoop && this.ogl._time.timeLoop(!0)
  ...
  init() {
    ...
    if (!document.querySelector("#fleet1 #warning")) {
      if (this.updateSpeedBar(), this.ogl.shipsList.forEach((e => {
        ...
        })), fleetDispatcher.selectMission(parseInt(fleetDispatcher.mission) || parseInt(this.ogl.db.options.defaultMission)), fleetDispatcher.speedPercent = fleetDispatcher.speedPercent || 10, fleetDispatcher.realSpeedPercent = fleetDispatcher.speedPercent, 1 === this.ogl.mode || 2 === this.ogl.mode ? (fleetDispatcher.selectShip(this.ogl.db.options.defaultShip, this.shipsForResources()), fleetDispatcher.selectMaxAll()) : 5 === this.ogl.mode && this.selectExpedition(this.ogl.db.lastExpeditionShip || this.ogl.db.options.defaultShip), fleetDispatcher.refresh(), fleetDispatcher.focusSubmitFleet1(), document.querySelector(".secondcol")) {
        Util.runAsync((() => {
  ...
  open() {
    ...
    if (this.blocks.forEach((n => {
        ...
        if (n.success > 0 ? o = Util.addDom("i", {
          ...
          const e = this.ogl._time.timeToHMS(this.ogl.db.options.useClientTime ? this.ogl._time.getClientTime(n.time) : n.time),
            t = Util.addDom("div", {
              class: "ogl_notificationLine",
              child: `${o.outerHTML}<b class="ogl_notificationTimer">[${e}]</b>` + n.message,
              prepend: this.content
            });
          n.success < 0 && t.classList.add("ogl_danger")
        }
        else t++
      })), t > 0 && Util.addDom("div", {
        class: "ogl_notificationLine",
...
class ShortcutManager extends Manager {
  load() {
    ...
    })), document.addEventListener("keyup", (() => this.locked = !1)), visualViewport.addEventListener("resize", (() => this.updateShortcutsPosition())), visualViewport.addEventListener("scroll", (() => this.updateShortcutsPosition()))), this.loaded = !0, this.add("menu", (() => (document.querySelector(".ogl_side.ogl_active .ogl_config") ? (this.ogl._ui.side.classList.remove("ogl_active"), delete this.ogl.db.currentSide) : this.ogl._topbar.openSettings(), !1))), this.add("showMenuResources", (() => (this.ogl.db.options.showMenuResources++, this.ogl.db.options.showMenuResources > 2 && (this.ogl.db.options.showMenuResources = 0), localStorage.setItem("ogl_menulayout", this.ogl.db.options.showMenuResources), document.body.setAttribute("data-menulayout", this.ogl.db.options.showMenuResources), !1))), this.add("previousPlanet", (() => {
  ...
  add(e, t, n, o) {
    ...
    "quickRaid" == e && (r.innerText += ` (${this.ogl.db.quickRaidList.length})`), this.keyList[a] = e => {
      ("fleet" != n || this.ogl._fleet.isReady && unsafeWindow.fleetDispatcher && !fleetDispatcher.fetchTargetPlayerDataTimeout) && i(e)
    }
  }
...
class TechManager extends Manager {
  ...
  getTechData(e, t, n) {
    ...
    const s = {};
    s.lifeform = a.lifeform || 0, this.totalLab = (a[31] || 0) + r;
    const d = {};
          ...
          if (o += c, 0 == r) {
            ...
            if (Util.addDom("time", {
              ...
              if (e.name = this.ogl.db.serverData[l] || l, e.lvl = s, e.end = o, e.type = t, n && n != this.ogl.currentPlanet.obj.coords) {
                ...
                this.ogl.db.myPlanets[o].upgrades = this.ogl.db.myPlanets[o].upgrades || [], this.ogl.db.myPlanets[o].upgrades[t] = [e]
              }
              else n == this.ogl.currentPlanet.obj.coords && "moon" != this.ogl.currentPlanet.obj.type && (this.ogl.currentPlanet.obj.upgrades = this.ogl.currentPlanet.obj.upgrades || [], this.ogl.currentPlanet.obj.upgrades[t] = [e])
            }
          }
          else if (r == a - 1) {
